...
export function BookListPage(props: BookListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBooks({
        relations: ['bookstore', 'inventory.count', 'order_item.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['title.contains', 'author.contains', 'isbn.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BookInterface>>(
    () => `/books?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'price', header: 'price', accessorKey: 'price' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.published_date ? format(parseISO(record?.published_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'isbn', header: 'Isbn', accessorKey: 'isbn' },
    hasAccess('bookstore', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'bookstore',
          header: 'Bookstore',
          accessorKey: 'bookstore',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/bookstores/view/${record.bookstore?.id}`}>
              {record.bookstore?.name}
            </Link>
          ),
        }
      : null,
