...
import { getBookstores, deleteBookstoreById } from 'apiSdk/bookstores';
import { BookstoreInterface } from 'interfaces/bookstore';

type ColumnType = ColumnDef<BookstoreInterface, unknown>;

interface BookstoreListPageProps {
...
export function BookstoreListPage(props: BookstoreListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBookstores({
        relations: ['user', 'book.count', 'inventory.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BookstoreInterface>>(
    () => `/bookstores?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BookstoreInterface) => {
    if (hasAccess('bookstore', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/bookstores/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/bookstores/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('bookstore', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/bookstores/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('bookstore', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
