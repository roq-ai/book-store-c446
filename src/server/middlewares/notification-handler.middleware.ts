...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.create': {
    roles: ['admin', 'book-manager'],
    key: 'new-customer-order',
    tenantPath: ['bookstore', 'user', 'order'],
    userPath: [],
  },
  'book.create': {
    roles: ['admin', 'book-manager'],
    key: 'book-added',
    tenantPath: ['bookstore', 'book'],
    userPath: [],
  },
  'book.delete': {
    roles: ['admin', 'book-manager'],
...
const tenantRoles: string[] = ['admin', 'book-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.bookstore.findFirst({
