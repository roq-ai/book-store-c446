...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['admin', 'book-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<BookstoreInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/bookstores` : null),
    () => getBookstores({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Bookstores',
      path: isTenantUser ? `/bookstores/view/${data?.[0]?.id}` : '/bookstores',
      entity: 'bookstore',
